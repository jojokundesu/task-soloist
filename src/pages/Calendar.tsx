...
const Calendar = () => {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(<div key={`empty-${i}`} className="h-14 bg-black/10 rounded-lg opacity-30"></div>);
    }
            ...
            <div className="mt-auto">
              <div className="w-full bg-black/30 rounded-full h-1 overflow-hidden">
                <div 
                  ...
                  style={{ width: `${(completedTasksCount / dateTasks.length) * 100}%` }}
                ></div>
              </div>
            </div>
          )}
      ...
      <div className="container mx-auto px-4 max-w-lg">
        ...
        <div className="flex items-center justify-between mb-6 animate-fade-in">
          ...
          <div className="flex items-center bg-black/20 rounded-lg px-3 py-1">
            ...
            <span className="text-sm">{selectedDate.toLocaleDateString(undefined, { month: 'short', year: 'numeric' })}</span>
          </div>
        </div>
        ...
        {/* Calendar Grid */}
        <div className="grid grid-cols-7 gap-2 mb-6 animate-scale-in">
          {renderCalendarDays()}
        </div>
            ...
            {selectedDateTasks.length > 0 ? (
              selectedDateTasks.map(task => (
                <div 
                  key={task.id}
                  className={cn(
                    "p-3 rounded-lg border flex items-center transition-all duration-300",
                    task.completed 
